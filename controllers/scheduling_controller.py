...
class Scheduling_controller:
    ...
    def _get_availability_string(self, scheduled_appointments):
        ...
        for date in sorted(unique_dates):
            ...
            available_from = self.WORK_START
            ...
            for (start, end) in sorted_times:
                ...
                available_from = datetime.strptime(end, self.TIME_FORMAT).time()
            
            if available_from != self.WORK_END:
                daily_availability.append(f"  - Available from: {format_time(available_from)} to {format_time(self.WORK_END)}")
